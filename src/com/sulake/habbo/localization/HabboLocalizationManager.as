package com.sulake.habbo.localization
{
   ...
   public class HabboLocalizationManager extends CoreLocalizationManager implements IHabboLocalizationManager
   {
      ...
      private var var_1756:Dictionary;
      ...
      {
         this.var_1756 = new Dictionary();
         this.var_2400 = ["I","II","III","IV","V","VI","VII","VIII","IX","X","XI","XII","XIII","XIV","XV","XVI","XVII","XVIII","XIX","XX"];
         super(param1,param2,param3);
         this.var_1757 = (param2 & 0) > 0;
         if(!this.var_1757)
         {
            lock();
         }
      ...
      private function onHabboConfigurationInit(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         if(param2 != null)
         {
            ...
            _loc3_ = "";
            _loc3_ = this.var_182.getKey("url.prefix",_loc3_);
            _loc3_ = _loc3_.replace("http://","");
            _loc3_ = _loc3_.replace("https://","");
            super.setPredefinedUrl(_loc3_);
            if(!this.var_1757)
            {
               if("false" == this.var_182.getKey("use.default.localizations","false"))
               {
                  _loc5_ = this.var_182.getKey("external.texts.txt","external_texts.txt");
                  events.addEventListener(LocalizationEvent.const_178,this.onLocalizationLoaded);
                  super.loadLocalizationFromURL(_loc5_);
               }
            }
            _loc4_ = 1;
            while(this.var_182.keyExists("localization." + _loc4_))
            {
               _loc6_ = this.var_182.getKey("localization." + _loc4_,"");
               _loc7_ = this.var_182.getKey("localization." + _loc4_ + ".name","");
               _loc8_ = this.var_182.getKey("localization." + _loc4_ + ".url","");
      ...
      {
         var _loc2_:* = null;
         events.removeEventListener(LocalizationEvent.const_178,this.onLocalizationLoaded);
         if(this.var_182.keyExists("external.override.texts.txt"))
         {
            _loc2_ = this.var_182.getKey("external.override.texts.txt");
            events.addEventListener(LocalizationEvent.const_178,this.onLocalizationOverrideLoaded);
      ...
      {
         events.removeEventListener(LocalizationEvent.const_178,this.onLocalizationOverrideLoaded);
         unlock();
      }
      ...
      {
         return this.var_2400[Math.max(0,param1 - 1)];
      }
   }
}
