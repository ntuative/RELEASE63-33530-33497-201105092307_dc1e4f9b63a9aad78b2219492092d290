package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         this.var_517 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_517 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2471 = _loc3_.localizationId;
         this.var_362 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_517,this);
               if(_loc5_)
               {
                  this.var_362 = _loc5_.colors;
         ...
         this._offers.add(this.var_517,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_797,this.onApproveNameResult);
      ...
      private function onWidgetsInitialized(param1:CatalogWidgetsInitializedEvent = null) : void
      {
         ...
         var _loc3_:* = [];
         for each(_loc4_ in this.var_362)
         {
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_57)
         {
            ...
            if(this.var_207 == null || _loc3_ >= this.var_207.length)
            {
               return;
            }
            this.var_775 = _loc3_;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_530 = param1.index;
         if(this.var_530 < 0 || this.var_530 > this.var_362.length)
         {
            this.var_530 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2471)
         {
            this.var_207 = this.parseSellableBreeds(param1.sellableBreeds);
            this.var_775 = 0;
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_517 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         {
            _loc5_ = this.var_207[_loc3_] as int;
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_517,_loc5_),this.getRaceLocalizationKey(this.var_517,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_207 == null || this.var_775 >= this.var_207.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_530 >= 0 && this.var_530 < this.var_362.length)
         {
            _loc7_ = this.var_362[this.var_530] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_207[this.var_775] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_517,_loc3_,_loc2_,AvatarScaleType.const_56,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_301,AvatarAction.const_738);
         _loc4_.endActionAppends();
         _loc4_.setDirection(AvatarSetType.const_35,2);
         this.setPreviewImage(_loc4_.getCroppedImage(AvatarSetType.const_35),true);
         _loc4_.dispose();
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_530 >= this.var_362.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_362[this.var_530] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_207[this.var_775] as int;
      ...
      private function updateAvailableBreeds(param1:String) : void
      {
         ...
         if(this.var_207 == null)
         {
            ...
            this.var_207 = this.parseSellableBreeds(_loc2_);
            this.var_775 = 0;
            if(this.var_207 != null)
            {
               this.updateBreedSelections();
