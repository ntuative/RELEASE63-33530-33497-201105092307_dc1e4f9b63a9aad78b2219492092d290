package com.sulake.habbo.roomevents.userdefinedroomevents.help
{
   ...
   public class UserDefinedRoomEventsHelp
   {
      ...
      private function prepareWindow() : void
      {
         ...
         this._window = IFrameWindow(this.var_80.getXmlWindow("ude_help"));
         this.var_1805 = IWindowContainer(this.find(this._window,"help_container"));
         this.var_1167 = this.var_1805.findChildByName("button_previous") as IContainerButtonWindow;
         if(this.var_1167)
         {
            this.var_1167.procedure = this.onButtonClick;
         }
         this.var_1166 = this.var_1805.findChildByName("button_next") as IContainerButtonWindow;
         if(this.var_1166)
         {
            this.var_1166.procedure = this.onButtonClick;
         }
      ...
      {
         if(this.var_252 >= this.var_57.length - 1)
         {
            this.var_1166.disable();
         }
         else
         {
            this.var_1166.enable();
         }
         if(this.var_252 <= 0)
         {
            this.var_1167.disable();
         }
         else
         {
            this.var_1167.enable();
         }
      }
      ...
      {
         ++this.var_252;
         this.var_252 = Math.min(this.var_252,this.var_57.length - 1);
         this.openPage();
      }
      ...
      {
         --this.var_252;
         this.var_252 = Math.max(this.var_252,0);
         this.openPage();
      }
      ...
      private function initLocalizations() : void
      {
         ...
         this.var_57 = [];
         var _loc1_:int = 1;
         while(this.var_80.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title"))
         {
            _loc2_ = this.var_80.localization.getLocalization("wiredfurni.help." + _loc1_ + ".title");
            _loc3_ = this.var_80.localization.getLocalization("wiredfurni.help." + _loc1_ + ".img");
            _loc4_ = this.var_80.localization.getLocalization("wiredfurni.help." + _loc1_ + ".text");
            this.var_57.push([_loc2_,_loc3_,_loc4_]);
            _loc1_++;
         }
      }
      ...
      private function localize() : void
      {
         ...
         if(!this.var_57 || this.var_57.length == 0)
         {
            return;
         }
         if(_loc1_)
         {
            _loc1_.text = ILocalization(this.var_57[this.var_252][0]).value;
         }
         if(_loc2_)
         {
            _loc2_.text = ILocalization(this.var_57[this.var_252][2]).value;
         }
         if(_loc3_)
         {
            _loc4_ = ILocalization(this.var_57[this.var_252][1]).value;
      ...
      {
         if(!param1 || !this.var_80 || !this.var_80.assets)
         {
            return;
         }
         var _loc2_:BitmapDataAsset = this.var_80.assets.getAssetByName(param1) as BitmapDataAsset;
         if(_loc2_ == null)
         {
      ...
      private function retrievePreviewAsset(param1:String) : void
      {
         ...
         var _loc2_:String = this.var_80.configuration.getKey("image.library.catalogue.url");
         var _loc3_:* = _loc2_ + param1 + ".gif";
         Logger.log("[ProductViewCatalogWidget] Retrieve Product Preview Asset: " + _loc3_);
         var _loc4_:URLRequest = new URLRequest(_loc3_);
         var _loc5_:AssetLoaderStruct = this.var_80.assets.loadAssetFromFile(param1,_loc4_,"image/gif");
         if(!_loc5_)
         {
            return;
         }
